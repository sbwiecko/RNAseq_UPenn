...
# code from script6 to grad the module of upregulated genes
...
gost.res.module <- gost(rownames(myModule), organism = "hsapiens", correction_method = "fdr")
gostplot(gost.res.module, interactive = F, capped = TRUE) #set interactive=FALSE to get plot for publications
...
c2cp <- read.gmt("/Users/Sébastien/Downloads/MSigDB/c2.cp.v7.1.symbols.gmt")
...
gseaplot2(myGSEA.res, 
          geneSetID = c(1,938), #can choose multiple signatures to overlay in this plot
...
ggplot(myGSEA.df[1:20,], aes(x=phenotype, y=ID)) + 
  geom_point(aes(size=setSize, # number of genes in the set
                 color = NES,
...
# both real and fake have some genes that are differentially expressed
