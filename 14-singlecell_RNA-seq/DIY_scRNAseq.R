...
# In the 1st 1/2 of the script, we'll practice some basics using a small (~1000 cell) dataset from
# human peripheral blood mononuclear cells (PBMCs). This dataset comes from the public datasets
# on the 10X Genomics website: https://www.10xgenomics.com/resources/datasets
# In the 2nd 1/2 of the script, we'll import two separate Seurat objects generated from
# the spleen of naive and Toxoplasma gondii infected mice, giving us an opportunity
# to create and analyze an integrated dataset
...
# a new directory called 'counts_filtered' is created that contains barcodes.tsv, genes.tsv and matrix.mtx


# Generate QA report ----
# this report will contain some simple but useful metrics as well as the traditional log-transformed
# UMI rank plot (a.k.a. 'waterfall' plot) plot was first described in the Drop-seq paper:
...
# This corresponds to a transition between a distinct subset of barcodes with large
...
# load filtered mtx if not loaded yet
#filt_mtx <- readMM('counts_filtered/matrix.mtx')

# load run info from JSON files produced by Kb
kb_stats <- c(
  fromJSON(file = "inspect.json"),
  fromJSON(file = "run_info.json")
)

# determine chemistry version by simply searching '10X' term
...
seq_stats <- data.frame(
  stat = c(
    ...
    "% reads on whitelist"   # % reads with barcode matching the whitelist
  ),
  value = prettyNum(
    c(
      tech,
...
p_cnts_in_cells <- round((sum(filt_mtx) / sum(raw_mtx)) * 100, 2) # ~UMIs
...
# output a HTML summary of the run in the 'counts_filtered' directory
print_HTML(
  seq_stats = seq_stats,
...
VlnPlot(
  ...
  c(
    ...
    "percent.mt"  # gives indication as to dead cells if pct > 20
  ),
  pt.size = 0.1
)
...
# generally a linear relationship is expected with a higher number of UMIs
# linked to a higher number of genes
...
#    These could be dying cells, **but** also could represent a population of a low
#    complexity celltype that express a low number of genes (i.e red blood cells).



# Plot UMAP ----
# Here PCA cannot be applied as there are many more samples, i.e., each single cell is one sample,
# compared to bulk RNA-Seq where 10-20 samples are analyzed. tSNE and UMAP are commly used in
# scRNA-Seq, with UMAP being currently the standard/best practice.
...
pbmc.1k.seurat <- ScaleData(pbmc.1k.seurat, verbose = TRUE)
pbmc.1k.seurat <- RunPCA(pbmc.1k.seurat, npcs = 40, verbose = TRUE)
...
# a PCA plot give a less complex graph and lower separation power with roughly 3 main clusters
...
# 1. 'FindMarkers' to **compare a select cluster to all other cells** not in that cluster (LFC)
# the genes classified as 'markers' need to be in 25% of the cells in the cluster
...
# pct.2 is the average in all other cells
...
pbmc.1k.markers <- FindAllMarkers(
  pbmc.1k.seurat,
  only.pos = TRUE,
  min.pct = 0.25,
...
# Usually we don't know the type of cells that consitute the different
# clusters, so we gonna try to identify the cell types based on the
# expression of genes, ultimately comparing the sets with public databases.
...
# there was an issue in the latest release of cellassign, that needed a manual fix,
# as discussed [here](https://github.com/Irrationone/cellassign/issues/92)
...
#    changing line 165 from `inference-tensorflow.R` to
#    p_y_on_c_norm <- tf$reshape(tf$reduce_logsumexp(p_y_on_c_unorm, 0L), as_tensor(shape(1,NULL)))
# 3. install.packages("devtools")
# 4. run devtools::install_github("sbwiecko/cellassign")
# Rtools is also required (https://cran.r-project.org/bin/windows/Rtools/rtools42/rtools.html)
library(cellassign)
...
# it can also be useful to turn the Seurat object into a singleCellExperiment object, for better
# interoperability with other bioconductor tools; conversion from/to Seurat objects possible.
...
# you can view this matrix as a heatmap to see overlapping genes
...
# run cellAssign, machine learning approach to predict identity of clusters
fit <- cellassign(
...
predictions <- SingleR(
  ...
  assay.type.test = 1,
  ref = Monaco.data,
  labels = Monaco.data$label.main
)

plotScoreHeatmap(predictions)

# we can look at another set
predictions.DICE <- SingleR(
  test = pbmc.1k.sce,
  assay.type.test = 1,
  ref = DICE.data,
  labels = DICE.data$label.main
)

plotScoreHeatmap(predictions.DICE)
...
# i.e., tag every cell in the Seurat object, adding a variable in the metadata slot
...
# seeing all the data integrated into a single UMAP
...
# going back to a Seurat object to give the treatment label
