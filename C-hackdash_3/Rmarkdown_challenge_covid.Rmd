...
We first looked at the original study design file, in particular by selecting and sorting the sample, cell_line, treatment (which contains the MOI) and timepoint_postTreatment columns. The study design reports a variety of different samples and conditions, including:

* human primary NHBE samples treated mock vs. SARS-CoV-2 (from now SARS), IAV (i.e. influenza A virus), IAVdNS1 or IFN-β for 24 h
...
* _in vivo_ nasal wash and trachea samples from ferrets treated mock vs. SARS (5E4 PFU) or IAV (1E5 PFU) for 1, 3, 7 or 14 days
* some other lung biopsies from patients

While no gene ontology is available for furret, the time course experiment _in vivo_ and the comparison with _ex vivo_ data using primary normal human bronchial epithelia (NHBE) cells seem to be a reasonable starting point for multivariate analysis of the gene signatures regulated during the infection with SARS-CoV-2. Moreover we can compare the gene signatures to the ones induced after infection with influenza A virus.  
The plan for the challenge is therefore:

1. classical RNAseq data analysis including filtering, normalization and PCA in both the human primary cells and ferret samples
...
The goal of this short script to it make sure we are able to read the count data and the study design for the COVID hackdash into the R environment using tidyverse
