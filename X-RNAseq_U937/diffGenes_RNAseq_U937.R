...
# code from step 3 to generate the DGEList.filtered.norm data
...
keepers <- rowSums(cpm(myDGEList) > 1) >= 5 #user defined
myDGEList.filtered <- myDGEList[keepers, ]

myDGEList.filtered.norm <- calcNormFactors(myDGEList.filtered, method = "TMM")

# now we start with the new code
...
ebFit <- eBayes(fits)
stats <- write.fit(ebFit, file = "./stats.txt")
...
# the function only give rows which data satisfy the criteria a +1 or -1
# depending on the direction of the change, and 0 if don't meet criteria
...
# be careful all of this is just about threshold

# retrieve expression data for your DEGs ----
head(v.DEGList.filtered.norm$E) # log2(counts) expression
colnames(v.DEGList.filtered.norm$E) <- sampleLabels

# subset where rows have the 1st col from results != 0
...
mySwitchList <- importRdata(
  ...
  # found if on http://www.ensembl.org/Homo_sapiens/Info/Index - Gene annotation - GTF
  ...
  showProgress             = TRUE) # Jaccard similarity error thrown, don't know yet what to do...
